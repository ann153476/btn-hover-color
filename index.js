...
const DRAW = () => {
  ...
  for (let t = 0; t < TRAILS.length; t++) {
    ...
    for (const NODE of TRAIL.nodes) {
      ...
      const RENDER_SIZE =
        TRAIL.nodes.length === 1 ? CONFIG.size : active * CONFIG.size;
      CONTEXT.beginPath();
      CONTEXT.arc(x, y, RENDER_SIZE * 0.5, 0, Math.PI * 2);
      CONTEXT.fill();
    }
  }
  if (CONFIG.active) {
    ...
    CONTEXT.beginPath();
    CONTEXT.arc(head.x, head.y, RENDER_SIZE * 0.5, 0, Math.PI * 2);
    CONTEXT.fill();
  }
};
...
const UPDATE_TRAIL = ({ x, y }) => {
  if (activeTrail.nodes.length > 0) {
    REMOVE_NODE(
      activeTrail.nodes[activeTrail.nodes.length - 1].id,
      activeTrail
    );
  }
  ADD_NODE({ x, y });
